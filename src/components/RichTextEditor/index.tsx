import React, { useEffect, useState } from "react";
import { Editor } from "@tinymce/tinymce-react";
import { EventHandler } from "@tinymce/tinymce-react/lib/cjs/main/ts/Events";
import { getSignedUrl } from "src/services";

interface IRichTextEditorProps {
  onChange: EventHandler<unknown>;
  initialValue: string;
}

const RichTextEditor: React.FC<IRichTextEditorProps> = ({
  onChange,
  initialValue
}) => {
  const [report, setReport] = useState<string | null>(initialValue);

  useEffect(() => {
    const getUrl = async () => {
      let reportWithCaptures = initialValue;
      const matches = [...initialValue.matchAll(/data-key='([^']+)'/g)];
      const keys = matches.map((match) => match[1]);

      await Promise.all(
        keys.map(async (key) => {
          const url = await getSignedUrl(key);
          const newSrc = `data-key='${key}' src=${url}`;
          reportWithCaptures = reportWithCaptures.replace(
            new RegExp(`data-key='${key}'`, "g"),
            newSrc
          );
        })
      );

      setReport(reportWithCaptures);
    };

    getUrl();
  }, [initialValue]);

  /*const editorRef = useRef(null);

  const handleEditorInit = (_event: any, editor: any) => {
    editorRef.current = editor;
  };

  const setReadOnlyMode = (readOnly) => {
    if (editorRef.current) {
      editorRef.current.mode.set(readOnly ? "readonly" : "design");
    }
  };*/

  return (
    <>
      <Editor
        apiKey={process.env.NEXT_PUBLIC_TINY_MCE_API_KEY}
        //onInit={handleEditorInit}
        init={{
          //width: 794,
          height: 600,
          plugins:
            "anchor autolink charmap image link lists media searchreplace table visualblocks wordcount",
          toolbar:
            "undo redo | blocks fontfamily fontsize | bold italic underline strikethrough | link image media table mergetags | align lineheight | tinycomments | checklist numlist bullist indent outdent | emoticons charmap | removeformat",
          tinycomments_mode: "embedded",
          tinycomments_author: "Author name",
          content_langs: [{ title: "French", code: "fr" }],
          language: "fr_FR",
          language_url: "/locales/fr/fr_FR.js",
          spellchecker_language: "fr_FR",
          content_style: "body { font-family: Arial; }"
          //menubar: false,
          //statusbar: false
        }}
        initialValue={report || ""}
        onChange={onChange}
      />
    </>
  );
};

export default RichTextEditor;
